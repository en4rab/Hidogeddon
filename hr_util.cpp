...
void SettingsInit(void) {
  ...
  preferences.begin("hrBLE", false);
  if (preferences.getBool("hrs_ok", false)) {
    hrSettings::bleMAC = preferences.getString("BLE_MAC", hrSettings::bleMAC);
    hrSettings::bleAddrType = preferences.getUInt("BLE_AT", hrSettings::bleAddrType);
  }
  preferences.end();
...
void SPIFFSInit(void) {
  if (SPIFFS.begin(true)) {
    hrSettings::logCount = TotalCardsInLog();
    return;
  }
...
void SaveBLESettings(void) {
  ...
  preferences.putUInt("BLE_AT", hrSettings::bleAddrType);
  preferences.putBool("hrs_ok", true);
  preferences.end();
}
...
void OLEDShowLogo(void) {
  if (!hrSettings::oledConnected) { return; }
  ...
  display.drawBitmap(0, 0, LOGO, hrSettings::SCREEN_WIDTH, hrSettings::SCREEN_HEIGHT, WHITE);
  display.display();
}
